...
def Sample1D(Hr, Hz):
    ...
    ResDict = {'Age': Age, 'Xkpc': X, 'Ykpc': Y, 'Zkpc': Z, 'Rkpc': R, 'Th': Th,
               'XRelkpc': XRel, 'YRelkpc': YRel, 'ZRelkpc': ZRel, 'RRelkpc': RRel}
...
def Sample1DPop(NComponent, Hr, Hz):
    ...
    RSet = np.asarray([RCDFInv(RRandSet[i], Hr)
                      for i in range(NComponent)], dtype=float)
    ZSet = np.asarray([zCDFInv(ZRandSet[i], Hz)
                      for i in range(NComponent)], dtype=float)*ZSignSet
    ...
    ResDict = {'ID': IDSet, 'Ages': AgeSet, 'Xkpc': XSet,
               'Ykpc': YSet, 'Zkpc': ZSet, 'Rkpc': RSet, 'Th': ThSet}
